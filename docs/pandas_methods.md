...
## Revised
| Method                         | Description | Anlysed | Override? | Done? | Tested?| Comment |
| -----------------------------  | ----------- | ------- | --------- | ----- | ----- | -------- |
| \_\_str\_\_ |  |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because __str__ is not call in pandas source.
| \_\_repr\_\_| |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because __str__ is not call in pandas source.
| DataFrame.head([n]) | Return the first n rows. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, even it's called in  call in ToCSVIndexes (csv.py)
| DataFrame.tail([n]) | Return the last n rows. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .tail is not call in pandas source.
 | *Serialization / IO / conversion* ||||| |
| DataFrame.to_orc([path, engine, index, ...]) | Write a DataFrame to the ORC format. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .tail is not call in pandas source.
| DataFrame.to_parquet([path, engine, ...]) | Write a DataFrame to the binary parquet format. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_parquet is not call in pandas source.
| DataFrame.to_pickle(path, *[, compression, ...]) | Pickle (serialize) object to file. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_pickle is not call in pandas source.
| DataFrame.to_hdf(path_or_buf, *, key[, ...]) | Write the contained data to an HDF5 file using HDFStore. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_hdf is not call in pandas source.
| DataFrame.to_sql(name, con, *[, schema, ...]) | Write records stored in a DataFrame to a SQL database. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_sql is not call in pandas source.
| DataFrame.to_excel(excel_writer, *[, ...]) | Write object to an Excel sheet. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_excel is not call in pandas source.
| DataFrame.to_json([path_or_buf, orient, ...]) | Convert the object to a JSON string. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_json is not call in pandas source.
| DataFrame.to_html([buf, columns, col_space, ...]) | Render a DataFrame as an HTML table. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_html is not call in pandas source.|
| DataFrame.to_feather(path, **kwargs) | Write a DataFrame to the binary Feather format. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_feather is not call in pandas source.
| DataFrame.to_latex([buf, columns, header, ...]) | Render object to a LaTeX tabular, longtable, or nested table. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_latex is not call in pandas source.|
| DataFrame.to_stata(path, *[, convert_dates, ...]) | Export DataFrame object to Stata dta format. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_stata is not call in pandas source.
| DataFrame.to_gbq(destination_table, *[, ...]) | (DEPRECATED) Write a DataFrame to a Google BigQuery table. |✔|❌|||
| DataFrame.to_clipboard(*[, excel, sep]) | Copy object to the system clipboard. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_clipboard is not call in pandas source.
| DataFrame.to_markdown([buf, mode, index, ...]) | Print DataFrame in Markdown-friendly format. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_markdown is not call in pandas source.
| DataFrame.to_csv([path_or_buf, sep, na_rep, ...]) | Write object to a comma-separated values (csv) file. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .to_csv is not call in pandas source.
| DataFrame.style | Returns a Styler object. |✔| ❌ |❌ |❌| Removing style attribute breaks, at least, read_csv |
| DataFrame.to_records([index, column_dtypes, ...]) | Convert DataFrame to a NumPy record array. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, because .records is not call in pandas source.
| DataFrame.to_string([buf, columns, ...]) | Render a DataFrame to a console-friendly tabular output. |✔| ✔ | ✔|✔| Raises AttributeError. Seem doesn't break code, it's only used in serialization function like repr, ...
| DataFrame.to_dict([orient, into, index]) | Convert the DataFrame to a dictionary. |✔|❌|❌|❌| It's used in .map|

## TODO To be revised

| Method                         | Description | Anlysed | Override? | Done? | Tested?|
| -----------------------------  | ----------- | ------- | --------- | ----- | ----- |
|*Constructor*|
